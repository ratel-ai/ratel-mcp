...
<div align="center">
  <img src="./docs/assets/hero.webp" width="960" alt="Ratel MCP server" />
</div>
...
- **Context layer** — sits between your MCP host (Claude Code, Cursor, ChatGPT) and your upstream MCP servers. Your agent sees only `search_capabilities` and `invoke_tool`, never the full tool list from every upstream.
...
# Point an agent at Ratel without removing native MCP entries
...
HTTP and SSE upstreams with OAuth drive a PKCE flow inline on `mcp add` and refresh automatically at server boot. Full OAuth reference: [docs.ratel.sh](https://docs.ratel.sh)

### `import` vs `link`

| Command | What it does | When to use |
|---|---|---|
| `mcp import` | Moves your MCP entries from Claude Code into `~/.ratel/config.json`. Claude Code connects through Ratel only. | The correct operational mode. Run this once to activate the filter. |
| `mcp link` | Adds the Ratel server to Claude Code while leaving native entries intact. Both paths remain active. | Exploratory only. The filter has no effect if tools still reach the model directly. |
...
When the server boots, it connects to every configured upstream MCP server and registers their tools into a Ratel `ToolCatalog`. The MCP host sees two tools: `search_capabilities` and `invoke_tool`. The full upstream catalog stays out of context until the agent searches for it.
...
| **Server** | [ratel-ai/ratel-mcp](https://github.com/ratel-ai/ratel-mcp) (this one) | MCP context layer for Claude Code, Cursor, and ChatGPT. No code changes needed. |
